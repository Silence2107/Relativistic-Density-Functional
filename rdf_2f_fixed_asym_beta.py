...
parser = argparse.ArgumentParser(description='2 flavour RDF with fixed baryon asymmetry in beta equilibrium',
...
required.add_argument('--asym', type=float, default=0.0,
                      help='Asymmetry parameter 3 * (n_d - n_u) / (n_d + n_u)')
...
def main():
    ...
    for count, mu_b_ef in enumerate(mu_b_ef_selection):
        ...
        # and 3 * (n_d - n_u) / (n_d + n_u) = args.asym to ensure proper asymmetry
        def fixed_asymmetry_eq(mu_q):
            ...
            # target asymmetry
            asym_target = densities[d_index] - densities[u_index]
            ...
            equation = (asym_target - args.asym * n_b) / \
                (0.16 / mev3_fm3)  # scaled to nsat
            return equation, [energy_density, pressure, n_b, [*densities, n_e], [*chempots, -mu_q], gap, q_bar_q]

        # optimize mu_q
        mu_q_opt = rdf.root(lambda x: fixed_asymmetry_eq(x)[
            0], guess_mu_q, tol=args.rel_tol)
        mu_q = mu_q_opt.x[0]
        if confirmed_SC:
            guess_mu_q = mu_q
        energy_density, pressure, n_b, densities, chempots, gap, q_bar_q = fixed_asymmetry_eq(mu_q)[
            1]
        if not args.non_verbose:
            print()
            print('Concluded iter. ', count + 1, '/', args.num_points, ', Mu ',
                  chempots, ' MeV, Error: ', mu_q_opt.fun, sep='', end='')
            if np.any(np.abs(mu_q_opt.fun) > args.rel_tol):
                print(', status:', mu_q_opt.message, end='')
            print()
